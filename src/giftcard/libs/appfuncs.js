...
var appFunc = {
    ...
    toBindCard: function() {
        ...
        if (isIos()) {
            // var goToHomeCommnd = ["", "APXInterface", "navigationBarUpdated", [title, type]];
            // window.webkit.messageHandlers.cordova.postMessage(goToHomeCommnd);
            apxInterface.toAppBindGiftCard(function(){
                if(window.reloadMyCardPage){
                    window.reloadMyCardPage();
                }
            })
        } else if (isAndroid()) { // 安卓的
            if (window.js2android.toAppBindGiftCard) {
                window.js2android.toAppBindGiftCard(); // 跳转到
            }
        // window.js2android.navigationBarUpdated(title, type); // 跳转到
        }
    },
