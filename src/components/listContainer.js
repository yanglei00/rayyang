...
class ListContainer extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            goodsCrads: [],
            listKey: 1,
            fetchParams: this.props.ajaxParams.data,
        };
    }
    componentDidMount() {
        //Cookies.remove('name');
        ...
        //     this.ajaxGoodsFun();
        // }
        // alert(sessionStorage.listScrollparams)
    ...
    ajaxGoodsFun = (type) => {
        ...
        console.log( this.props.ajaxParams )
        ...
        // }
        let propsData = this.state.fetchParams;//this.props.ajaxParams.data
        send_data = {
            ...send_data,
            ...propsData
        }
        if( propsData.type == 3 && this.state.pageIndex != 1 && !this.state.isLoadLoveed ){
            send_data.pageIndex = 1;
            this.setState({
                pageIndex:1,
                isLoadLoveed:true
            })
        ...
        fetch(this.props.ajaxParams.url, {
            ...
            .then((response) => {
                ...
                if( this.props.loadType == 'index' ){

                    if( send_data.type == 3 ){
                        var newData = [];
                        newData[0] = {
                            bannerData:false,
                            data:response[this.props.ajaxParams.rsDatakey][0].data.productInfo.list,
                            title:{title:"猜你喜欢",moreUrl:''},
                            pageIndex:response[this.props.ajaxParams.rsDatakey][0].data.productInfo.pageIndex,
                            totalPage:response[this.props.ajaxParams.rsDatakey][0].data.productInfo.totalPage,
                            type:3
                        }
                        rsDataArray = [newData];
                        if ( newData[0].pageIndex == newData[0].totalPage) {
                            isHasMore = false;
                        }
                ...
                // 从商品详情返回列表定位
                if (sessionStorage.listScrollparams){
                    commonFn.listScrollBackPosition('am-list-view-scrollview', 'get', '', (fetchParams, callback) => {
                        // console.log(fetchParams)
                        // console.log(this.state.fetchParams)
                        if (JSON.stringify(this.state.fetchParams) != JSON.stringify(fetchParams.data)){
                            this.setState({
    ...
    componentWillReceiveProps(nextProps) {
        ...
        this.setState({
            fetchParams:nextProps.ajaxParams.data
        })
    ...
    onScroll(e) {
        ...
        if (this.props.onScroll && e.scroller) {
            ...
            console.log(e.scroller.__scrollTop)
    ...
    renderRowFunc = (rowData, sectionID, rowID) => {
        ...
        return (
            <div key={rowID} className="row-container" >
                {this.props.rowTpl( rowData, this.state.pageIndex-1,this.state.fetchParams.type,this.state.fetchParams.categoryId )}
            </div>
        );
    }
        ...
        return (
            <div className={`list-view-container`}>
                <ListView
                    ...
                    style={{
                        height: this.props.useBodyScroll?'auto':(this.props.listViewWrapHeight ? this.props.listViewWrapHeight : document.body.clientHeight - 76),
                    }}
